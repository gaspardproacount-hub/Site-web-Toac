...
export async function generateDechargePdf(
  ...
  // `showInWindowTitleBar` demande aux lecteurs d'afficher ce titre plutôt que
  // le nom du fichier — sans quoi l'onglet du navigateur montre « documents »,
  // le dernier segment de l'URL qui sert le PDF.
  pdfDoc.setTitle(buildDechargeDocumentTitle(data.nom, data.prenom), {
    showInWindowTitleBar: true,
  });
  pdfDoc.setSubject("Décharge salle de musculation et certificat médical");
  pdfDoc.setAuthor(`${data.prenom} ${data.nom}`);
